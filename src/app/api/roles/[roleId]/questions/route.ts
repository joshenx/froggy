...
import { createBankQuestion } from "@/lib/mvp/store";
import { readJson, serverError } from "@/lib/mvp/http";
import type { CreateQuestionInput, Difficulty, RoleLevel } from "@/lib/mvp/types";
...
export async function POST(
  ...
  try {
    ...
    const body = (await readJson(request)) as Partial<CreateQuestionInput>;
    ...
    const question = await createBankQuestion(roleId, {
      title: body.title,
      prompt: body.prompt,
